...
- **EX-POST ウォッチ層（進行中ケースのニュース事後反映）/ 2026-06-28 方向性合意**: 進行中の戦争（例: ウクライナ 2022-）の現在ニュースを**定期自動更新**で取り込む構想。位置づけは「事後ウォッチ層（分離）」で確定。**不可侵の制約**:
  - pre-war 評価セル・`形跡`・`rating`・`exAnteEvaluability` には**一切触れない**（第3原則＝開戦後証拠を開戦前判断の直接証拠にしない）。目的は勝敗判定でなく、(a)「開戦前に flag したリスクが事後に顕在化したか」(b)「説明責任 claim の事後検証」のみ。§F-L の昇格条件「後年資料で当該リスクが顕在化」と接続するが、昇格は人手レビュー前提。
  - データは pre-war 監査データと**物理分離**（別ファイル/別名前空間。例 `data/watch/<caseId>.js` か `exPostWatch[]`）。renderer も専用パネルでラベルを「事後・参考」と明示。
  - **アーキテクチャ**: 本アプリはバックエンド無しの静的 JS。実行時 fetch 不可。→ スケジュール実行（GitHub Actions / cron）で fetch→整形→別データに commit。fetch・整形・保存は `tools/` でスクリプト化（[[スクリプト優先]]）。ただし**どのニュースが監査上関連し、どう分類するかは判断**＝全自動取り込みにせず人/モデルを噛ませる（`data/cases` 改変を機械任せにしない規律に準ずる）。
  - 着手は独立セッション。CANON へは複数ケースで有用性が確認されてから昇格。

export const francoPrussianWarPrussiaCase = {
  warCase: {
    ...
    uncertainty: "中",
    rating: "B/B-",
    ratingNote: "能力肯定面（外交誘導・動員優位）は強い一方、戦争長期化リスクとアルザス＝ロレーヌ併合コストには同時代に可視だった警告があり、併合コストは重大懸念として残る。出典精度も文書番号・頁レベルでは未精査のため、勝者バイアスを避けて B+/B ではなく暫定 B/B- に抑える。条件付きノックアウト基準（併合コスト軽視の確定＝上限B-、長期化の事前不織込みの確定＝上限B）を設定したが、対象セルは要検証に留まるため現状いずれも未発火である。"
  },
  ...
  // B/B- は確定だが、対象セルは戦後設計・長期化リスクで要検証に留まるため条件付き上限のみ（未発火）。
  ratingRules: {
    knockoutCriteria: [
      {
        id: "ko_annexation_cost_critical",
        cellIds: ["fpwp_cell_annexation_cost"],
        applies: false,
        condition:
          "アルザス・ロレーヌ併合がもたらす対仏敵対の長期コストを、講和時点で可視だった同時代の警告・反対に照らして衡量しなかったことが、内部審議記録で確定した場合。",
        capRatingAt: "B-",
        rationale:
          "戦後体制設計の最大級コストが致命的欠落と確定すれば、外交誘導・動員優位の強さでは相殺できない。現状は回顧録依拠を含み要検証であり、本基準は未発火。",
      },
      {
        id: "ko_protraction_unplanned",
        cellIds: ["fpwp_cell_siege_escalation"],
        applies: false,
        condition:
          "国民戦争・長期包囲という長期化局面を開戦前の作戦計画が想定せず、兵站・外交的カバーを欠いたことが一次資料で確定した場合。",
        capRatingAt: "B",
        rationale:
          "長期化の事前不織込みが致命的欠落と確定すれば B 圏上限を維持できない。現状は支持・反証が両建てで要検証であり、本基準は未発火。",
      },
    ],
    weightedScore: {
      enabled: false,
